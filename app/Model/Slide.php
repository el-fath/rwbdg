...
class Slide extends Model
{
    ...
    {
        $url = url('/')."/public/image/slide/".$this->image;
        return $url;
    }

    public function ImagePathLocal()
    {
        $url = public_path("image/slide/".$this->image);
        return $url;
    }
    ...
    public function getImagePathSmallAttribute(){
            ...
            if(isset($path_parts['filename'])){
                ...
                $urlImage = public_path("image/slide/".$path_parts['filename']."_small.png");
                // dd($urlImage);
                ...
                if(!$fileexist){
                    ...
                    $img->save('public/image/slide/'.$path_parts['filename']."_small.png",50);
                    // $img->encode('png');
                    // $type = 'png';
                ...
                return url('/')."/public/image/slide/".$this->image;
            }
    }
    ...
    public function getImagePathMediumAttribute(){
        ...
        if(isset($path_parts['filename'])){
            ...
            $urlImage = public_path("image/slide/".$path_parts['filename']."_medium.png");
            // dd($urlImage);
            $fileexist = File::exists($urlImage);
            if(!$fileexist){
                ...
                $img->save('public/image/slide/'.$path_parts['filename']."_medium.png",50);
                // $img->encode('png');
                // $type = 'png';
                // $base64 = 'data:image/' . $type . ';base64,' . base64_encode($img);
            }
            return url('/')."/public/image/slide/".$path_parts['filename']."_medium.png";
        }else{
            return url('/')."/public/image/slide/".$this->image;
        }
    }
